  ...
  function ProfileService(CONSTANTS, ApiService, UserService, $q, $filter, $rootScope) {
    ...
    function getRanks(stats) {
      ...
      function removeRanklessNoSubmissions(arr) {
        return arr.filter(function(subTrack) {
          return subTrack && ((subTrack.track === 'DESIGN' && subTrack.challenges) ||
            subTrack.rank || subTrack.rating || subTrack.wins ||
            subTrack.fulfillment || subTrack.submissions);
        });
      }
    ...
    function sortByDate(arr) {
      arr.sort(function(a, b) {
        if (!(a.mostRecentSubmission || a.mostRecentEventDate)) {
          return -1;
        }

        if (!(b.mostRecentSubmission || b.mostRecentEventDate)) {
          return 1;
        }
      ...
      function toObject(array) {
        ...
        array.forEach(function(level) {
          ...
          level.problemsSuccessful = (
            level.problemsSubmitted - level.problemsFailed - (level.problemsSysByTest || 0)
            ) || 0;
        ...
        ans.levels = [];
        if (ans['Level One']) {
          ans.levels.push(ans['Level One']);
        }

        if (ans['Level Two']) {
          ans.levels.push(ans['Level Two']);
        }

        if (ans['Level Three']) {
          ans.levels.push(ans['Level Three']);
        }
